...
import { scoreColor, scoreColor as sc, formatEur } from "@/lib/utils";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `Best Investment Opportunities in ${m.name}, ${m.region} 2026`,
    description: `${m.name} scores ${m.opportunity_score}/100 on the prime-atlas Opportunity Index. Discover ranked investment opportunities, infrastructure projects, and live signals in ${m.name}, ${m.region}, Spain.`,
    openGraph: {
      title: `Investment Opportunities in ${m.name} | prime-atlas`,
      description: `${m.name} Opportunity Score: ${m.opportunity_score}/100. AI-generated investment theses, infrastructure signals, and ranked opportunities.`,
    },
  };
}
...
export default async function MunicipalityPage({ params }: PageProps) {
  ...
  const jsonLd = {
    ...
    description: `Investment opportunity analysis for ${municipality.name}, ${municipality.region}, Spain`,
  ...
  return (
    <>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6 mb-10">
          <div>
            <p className="text-xs text-muted-foreground font-mono uppercase tracking-widest mb-2">
              {municipality.region} · Spain · Pop. {municipality.population.toLocaleString("en-GB")}
            </p>
            <h1 className="text-4xl font-bold mb-2">
              Investment Opportunities in {municipality.name}
            </h1>
            <p className="text-muted-foreground text-sm">
              prime-atlas Opportunity Score · Updated weekly
            </p>
          </div>
        ...
        <div className="border border-border rounded-xl p-6 bg-card mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-4">
            ...
            <div className="space-y-3 flex flex-col justify-center">
              ...
              <ScoreBar label="Safety (inverted risk)" score={100 - municipality.risk_score} />
            </div>
          </div>
        ...
        {/* AI Thesis — public teaser, prompts upgrade for full generation */}
        <div className="border border-border rounded-xl p-6 bg-card mb-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section>
              ...
              {opportunities?.length ? (
                ...
                <p className="text-sm text-muted-foreground">No active opportunities scored yet.</p>
              )}
            </section>
            ...
            {infraProjects && infraProjects.length > 0 && (
              <section>
                <h2 className="font-semibold mb-4">
                  Infrastructure Projects
                  <span className="ml-2 text-xs text-muted-foreground font-normal">({infraProjects.length})</span>
                </h2>
                  ...
                  {(infraProjects as InfrastructureProject[]).map((proj) => (
                    <div key={proj.id} className="border border-border rounded-xl p-4 bg-card">
                      ...
                      <p className="text-xs text-muted-foreground mt-2">
                        Budget: {formatEur(proj.budget / 100)} · Expected: {proj.expected_completion ? new Date(proj.expected_completion).getFullYear() : "TBC"}
                      </p>
                    </div>
                  ))}
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="border border-border rounded-xl bg-card overflow-hidden">
              <div className="px-4 py-3 border-b border-border flex items-center justify-between">
                <h3 className="font-semibold text-sm">Live Signals</h3>
              ...
              <div className="divide-y divide-border">
                {signals?.length ? (
                  ...
                  <p className="p-4 text-xs text-muted-foreground">No signals yet.</p>
                )}
              </div>
            </div>
        ...
        <section className="mt-12 pt-8 border-t border-border">
          <h2 className="font-semibold text-sm mb-4">Explore more in {municipality.region}</h2>
          <div className="flex gap-3 flex-wrap">
            <Link href="/rankings" className="text-xs text-pa-green hover:underline">Spain Opportunity Index →</Link>
